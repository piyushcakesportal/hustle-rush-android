...
public final class HustleRushView extends View {
    ...
    private static final String PREF_UNLOCKED_LEVEL = "unlocked_level";
    private static final String PREF_SELECTED_LEVEL = "selected_level";
    private static final String PREF_LEVEL_STARS = "level_stars_";
    private static final int ENDLESS_INDEX = 10;
    private static final String[] LEVEL_NAMES = {
            "FIRST PAYCHECK", "RENT WEEK", "EMI RUSH", "TAX SEASON", "FUEL CRISIS",
    ...
    public HustleRushView(Context context) {
        ...
        unlockedLevel = clampInt(preferences.getInt(PREF_UNLOCKED_LEVEL, 0), 0, ENDLESS_INDEX);
        selectedLevel = clampInt(preferences.getInt(PREF_SELECTED_LEVEL, unlockedLevel), 0, unlockedLevel);
    ...
    private void updateGame(float dt) {
        ...
        if (screen == Screen.PLAYING && activeLevel < ENDLESS_INDEX
                && distance >= LEVEL_TARGETS[activeLevel]) {
            completeLevel();
        }
    }
    ...
    private void completeLevel() {
        ...
        setKeepScreenOn(false);
        levelCompleteStars = 1;
        if (runBestCombo >= 4) levelCompleteStars++;
        if (cash >= 750 + activeLevel * 75) levelCompleteStars++;

        int previousStars = preferences.getInt(PREF_LEVEL_STARS + activeLevel, 0);
        int nextUnlocked = Math.min(ENDLESS_INDEX, Math.max(unlockedLevel, activeLevel + 1));
        unlockedLevel = nextUnlocked;
        preferences.edit()
                .putInt(PREF_LEVEL_STARS + activeLevel, Math.max(previousStars, levelCompleteStars))
                .putInt(PREF_UNLOCKED_LEVEL, unlockedLevel)
                .apply();
        if (runPeak > allTimeBest) {
            allTimeBest = runPeak;
            preferences.edit().putInt(PREF_BEST, allTimeBest).apply();
        }
        addBurst(playerX, playerY, cashGreen, 30);
        playShieldSound();
        vibrate(180, 180);
        invalidate();
    }
    ...
    private void drawMenu(Canvas canvas) {
        float safeTop = topInset + dp(22f);
        drawLogo(canvas, width / 2f, safeTop + dp(42f), 0.88f);
        drawCenteredText(canvas, "HUSTLE RUSH", width / 2f, safeTop + dp(103f), dp(29f), textPrimary, true);
        drawCenteredText(canvas, "Complete missions. Build your fortune.", width / 2f, safeTop + dp(129f), dp(13f), textMuted, false);

        float cardTop = safeTop + dp(154f);
        RectF missionCard = new RectF(dp(22f), cardTop, width - dp(22f), cardTop + dp(126f));
        drawPanel(canvas, missionCard, dp(24f), panel, Color.argb(80, 140, 124, 255));
        levelLeftButton.set(missionCard.left + dp(12f), missionCard.top + dp(37f), missionCard.left + dp(56f), missionCard.top + dp(81f));
        levelRightButton.set(missionCard.right - dp(56f), missionCard.top + dp(37f), missionCard.right - dp(12f), missionCard.top + dp(81f));
        drawCircleButton(canvas, levelLeftButton, "‹");
        drawCircleButton(canvas, levelRightButton, "›");
        ...
        drawGradientButton(canvas, primaryButton, selectedLevel == ENDLESS_INDEX ? "START ENDLESS RUN" : "START LEVEL " + (selectedLevel + 1), "▶", accent, accentLight);
        ...
        float badgeY = Math.min(height - bottomInset - dp(30f), toggleTop + dp(75f));
        drawPill(canvas, width / 2f, badgeY, "10 MISSIONS  •  ENDLESS CITY  •  NO LOGIN");
    }
    ...
    private void drawGameHud(Canvas canvas) {
        ...
        String progress = activeLevel == ENDLESS_INDEX
    ...
    private void drawLevelCompleteOverlay(Canvas canvas) {
        paint.setColor(Color.argb(210, 5, 8, 20));
        canvas.drawRect(0, 0, width, height, paint);
        float cardWidth = Math.min(width - dp(36f), dp(390f));
        float left = (width - cardWidth) / 2f;
        float top = height / 2f - dp(220f);
        RectF card = new RectF(left, top, left + cardWidth, top + dp(440f));
        drawPanel(canvas, card, dp(30f), panel, Color.argb(120, 50, 213, 154));

        paint.setColor(Color.argb(45, 50, 213, 154));
        canvas.drawCircle(width / 2f, top + dp(68f), dp(48f), paint);
        paint.setColor(cashGreen);
        canvas.drawCircle(width / 2f, top + dp(68f), dp(34f), paint);
        drawCenteredText(canvas, "✓", width / 2f, top + dp(78f), dp(31f), Color.WHITE, true);
        drawCenteredText(canvas, "LEVEL COMPLETE", width / 2f, top + dp(145f), dp(25f), textPrimary, true);
        drawCenteredText(canvas, LEVEL_NAMES[activeLevel], width / 2f, top + dp(174f), dp(13f), accentLight, true);
        drawCenteredText(canvas, starsText(levelCompleteStars), width / 2f, top + dp(218f), dp(28f), warning, true);

        drawCenteredText(canvas, Math.round(distance) + " m  •  ₹" + cash + "  •  Combo x" + runBestCombo,
                width / 2f, top + dp(258f), dp(12.5f), textMuted, false);
        String starHint = levelCompleteStars == 3
                ? "Perfect hustle! All mission stars earned."
                : "Earn more cash and reach combo x4 for 3 stars.";
        drawCenteredText(canvas, starHint, width / 2f, top + dp(289f), dp(11.5f), textMuted, false);

        primaryButton.set(left + dp(18f), top + dp(320f), card.right - dp(18f), top + dp(378f));
        homeButton.set(left + dp(18f), top + dp(390f), card.right - dp(18f), top + dp(430f));
        drawGradientButton(canvas, primaryButton,
                activeLevel < 9 ? "NEXT LEVEL" : "UNLOCK ENDLESS CITY", "▶", accent, accentLight);
    ...
    private void drawHowTo(Canvas canvas) {
        ...
        drawText(canvas, "MISSION GOAL", tipCard.left + dp(18f), tipCard.top + dp(25f), dp(11f), warning, true, Paint.Align.LEFT);
        drawText(canvas, "Reach the target distance without emptying your wallet. Complete all 10 missions to unlock Endless City.", tipCard.left + dp(18f), tipCard.top + dp(49f), dp(12.5f), textPrimary, false, Paint.Align.LEFT, tipCard.width() - dp(36f));
    }
    ...
    private void drawPrivacy(Canvas canvas) {
        ...
        drawText(canvas, "Stored on your device:", x, y, dp(15f), textPrimary, true, Paint.Align.LEFT);
        y += dp(34f);
        drawBullet(canvas, x, y, "Scores, unlocked levels and stars");
        y += dp(31f);
    ...
    private void handleTapOrSwipe(float x, float y) {
        ...
        if (screen == Screen.MENU) {
            ...
            else if (levelLeftButton.contains(x, y)) selectLevel(-1);
            else if (levelRightButton.contains(x, y)) selectLevel(1);
    ...
    private void selectLevel(int direction) {
        selectedLevel = clampInt(selectedLevel + direction, 0, unlockedLevel);
        saveSelectedLevel();
        vibrate(12, 45);
    }
    ...
    public boolean handleBackPressed() {
        ...
        if (screen == Screen.PAUSED || screen == Screen.GAME_OVER || screen == Screen.LEVEL_COMPLETE
                || screen == Screen.HOW_TO || screen == Screen.PRIVACY) {
            goHome();
            invalidate();
            return true;
        }
